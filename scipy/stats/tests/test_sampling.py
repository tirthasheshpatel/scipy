...
class TestTransformedDensityRejection:
    ...
    def test_sampling(self, dist, dpdf, params):
        ...
        rng = TransformedDensityRejection(pdf, dpdf, params=params,
                                          domain=domain, seed=123)
        rvs = rng.rvs(100_000)
    ...
    @pytest.mark.parametrize("bad_pdf, msg",
        [
            (lambda x: foo, r"name 'foo' is not defined"),
            (lambda x, a, b: x + a + b, r"missing 2 required positional "
                                        r"arguments: 'a' and 'b'"),
            ...
            (lambda x: x*x/2 + 2, r"51 : dTfx0 < dTfx1 \(x0<x1\). PDF not "
                                  r"T-concave!"),
            (lambda: 1., r"takes 0 positional arguments but 1 was given"),
            (None, r"`pdf` must be a Python callable")
        ]
    )
    @pytest.mark.parametrize("kwargs",
        [{"dpdf": (lambda x: x), "domain": (1, 10)}]
    )
    def test_bad_pdf(self, bad_pdf, msg, kwargs):
        with pytest.raises(Exception, match=msg):
            TransformedDensityRejection(bad_pdf, **kwargs)

    @pytest.mark.parametrize("bad_dpdf, msg",
        [
            (lambda x: foo, r"name 'foo' is not defined"),
            (lambda x: x*x, r"51 : hat\(x\) < PDF\(x\)"),
            (lambda x: None, r"must be real number, not NoneType"),
            ...
            (lambda x: np.nan, r"51 : cannot create bounded hat!"),
            (lambda: 1., r"takes 0 positional arguments but 1 was given"),
            (None, r"`dpdf` must be a Python callable")
        ]
    )
    @pytest.mark.parametrize("kwargs",
        [{"pdf": (lambda x: x*x/2), "domain": (1, 10)}]
    )
    ...
    def test_bad_domain(self, domain):
        with pytest.raises(ValueError, match=r"left >= right"):
            TransformedDensityRejection(lambda x: x, lambda x: 1,
                                        domain=domain)
    ...
    def test_bad_cpoints(self):
        ...
        with pytest.warns(UserWarning, match=r"number of starting points < 0"):
            TransformedDensityRejection(lambda x: x, lambda x: 1,
                                        domain=(0, 10), cpoints=-10)

        with pytest.warns(UserWarning, match=r"hat/squeeze ratio too small"):
            TransformedDensityRejection(lambda x: 1-x*x, lambda x: -2*x,
                                        domain=(-1,1), cpoints=1)
...
class TestDiscreteAliasUrn:
    ...
    def test_sampling_with_pmf(self, dist, params):
        ...
        assert_allclose(pval, 1., atol=1e-3)

    @pytest.mark.parametrize("bad_pmf, msg",
        [
            (lambda x: foo, r"name 'foo' is not defined"),
            (lambda x, a, b: x + a + b, r"missing 2 required positional "
                                        r"arguments: 'a' and 'b'"),
            ...
            (lambda x: 0., r"240 : unknown error"),
            (lambda: 1., r"takes 0 positional arguments but 1 was given"),
            (None, r"Expected either a callable or an array_like, recieved "
                   r"None.")
        ]
    )
    ...
    def test_bad_urnfactor(self):
        with pytest.warns(UserWarning, match=r"relative urn size < 1."):
            DiscreteAliasUrn([0.5, 0.5], urnfactor=-1)
