...
#define UNURAN_THUNK(CAST_FUNC, FUNCNAME, LEN)                                              \
    ...
    /* If an error has occured, return NAN. */                                              \
    if (PyErr_Occurred()) return NAN;                                                       \
    ...
    if (error) {                                                                            \
        /* nonlocal return causes memory leaks. So, if the Python error variable has been   \
           set, just return NAN. This will cause an internal error in UNU.RAN and it        \
           will return an errorcode or NULL value. We can then raise the error from Cython  \
           by checking if the Python error variable has been set (PyErr_Occurred) when an   \
...
void
error_handler(const char *objid, const char *file, int line,
    const char *errortype, int unur_errno, const char *reason)
{
    if ( unur_errno != UNUR_SUCCESS ) {
        ...
        (objid == NULL || strcmp(objid, "") == 0) ?
            strcpy(objid_, "unknown") : strcpy(objid_, objid);
        (reason == NULL || strcmp(reason, "") == 0) ?
            strcpy(reason_, "unknown error!") : strcpy(reason_, reason);

        const char *errno_msg = unur_get_strerror(unur_errno);

        if ( strcmp(errortype, "error") == 0 ) {
            fprintf(LOG, "[objid: %s] %d : %s => %s", objid_, unur_errno,
                reason_, errno_msg);
        }
        else {
            PyErr_WarnFormat(PyExc_RuntimeWarning, 2,
                "[objid: %s] %d : %s => %s",
                objid_, unur_errno, reason_, errno_msg);
        }
    }
}
...
int
init_unuran_callback(ccallback_t *callback, PyObject *fcn)
{
    int ret;
...
int
release_unuran_callback(ccallback_t *callback)
{
...
double
pmf_thunk(int x, const struct unur_distr *distr)
{
    UNURAN_THUNK(PyLong_FromLong, "pmf", 3);
}

double
pdf_thunk(double x, const struct unur_distr *distr)
{
    UNURAN_THUNK(PyFloat_FromDouble, "pdf", 3);
}

double
dpdf_thunk(double x, const struct unur_distr *distr)
{
    UNURAN_THUNK(PyFloat_FromDouble, "dpdf", 4);
}

double
cont_cdf_thunk(double x, const struct unur_distr *distr)
{
    UNURAN_THUNK(PyFloat_FromDouble, "cdf", 3);
}

double
discr_cdf_thunk(int x, const struct unur_distr *distr)
{
